...
class TodosAPITestCase(APITestCase):
    ...
    def authenticate(self):
        ...
        response = self.client.post(reverse("register"), {'username': "username",
                                                          "email": "email@gmail.com",
                                                          "password": "password"})
        self.client.credentials(HTTP_AUTHORISATION=f"Barer {response.data['token']}")
...
class TestListCreate(TodosAPITestCase):
    ...
    def test_retrieves_all_todos(self):
        ...
        self.assertEqual(response.data['results', list])
        self.create_todo()

        res = self.client.post(reverse('todos'))
